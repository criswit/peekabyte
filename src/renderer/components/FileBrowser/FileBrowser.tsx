...
import { FileItem, Favorite } from '@shared/types';
...
const FileBrowser: React.FC<FileBrowserProps> = ({
    ...
    const initializeHomeDir = async () => {
      try {
        ...
        if (window.electronAPI) {
          ...
          setError(
            'Electron API not available. This app requires Electron to access the file system.'
          );
        }
        ...
        const savedShowDotFiles = localStorage.getItem(
          'markdownViewerShowDotFiles'
        );
  ...
  }, [showDotFiles, currentPath, loadDirectoryContents]);
  ...
  const loadDirectoryContents = useCallback(
    async (dirPath: string) => {
      try {
        ...
        const dirContents = await window.electronAPI.readDirectory(
          dirPath,
          showDotFiles
        );
        ...
        if (
          ...
          if (parentPath && parentPath !== dirPath) {
            console.log(
              `Directory not found, navigating to parent: ${parentPath}`
            );
  ...
  const handleFileClick = async (file: FileItem) => {
    if (file.isDirectory) {
    ...
    } else if (file.isMarkdown || file.isJson || file.isHtml || file.isCsv) {
      try {
        ...
        // The viewer will handle displaying the error
        onFileSelect({
          ...file,
          content: '',
      ...
      try {
        ...
        const content = await window.electronAPI.readBinaryFile(file.path);
  ...
  return (
    <FileBrowserContainer>
      <FavoritesSection>
        <FavoritesHeader>
          <span>Favorites</span>
          <span onClick={toggleFavoritesSection} style={{ cursor: 'pointer' }}>
            {showFavorites ? '▼' : '►'}
          </span>
        </FavoritesHeader>
      ...
      {error ? (
        ...
        <div style={{ padding: '10px', color: '#ccc', textAlign: 'center' }}>
          Loading...
        </div>
      ) : (
        <FileList>
          {files.map(file => (
            ...
            >
              {file.isDirectory ? (
                file.isSymlink ? (
                  <SymlinkIcon title='Symbolic Link to Directory'>
                    🔗📁
                  </SymlinkIcon>
                ) : (
                  <FolderIcon>📁</FolderIcon>
                ...
                file.isSymlink ? (
                  <SymlinkIcon title='Symbolic Link to Markdown File'>
                    🔗📄
                  </SymlinkIcon>
                ) : (
                ...
                file.isSymlink ? (
                  <SymlinkIcon title='Symbolic Link to JSON File'>
                    🔗📋
                  </SymlinkIcon>
                ) : (
                  <FileIcon style={{ color: 'var(--json-icon-color)' }}>
                    📋
                  </FileIcon>
                )
              ) : file.isSymlink ? (
