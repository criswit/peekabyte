...
const SourceViewWithHighlights: React.FC<SourceViewWithHighlightsProps> = ({
  ...
  useEffect(() => {
    ...
    if (matches.length > 0 && containerRef.current) {
      setTimeout(() => {
        const currentMatch =
          containerRef.current?.querySelector('.current-match');
        if (currentMatch) {
...
const MarkdownViewer: React.FC<MarkdownViewerProps> = ({
  ...
  // Set match refs
  const setMatchRefs = useCallback(
  ...
  const handleRefreshClick = async () => {
    ...
    try {
      ...
      const updatedContent = await window.electronAPI.readFile(
        selectedFile.path
      );
  ...
  const components = useMemo<Components>(() => {
    ...
    const createComponentWrapper = (type: string) => {
      const ComponentWrapper = ({ children, ...props }: any) => {
      ...
      ComponentWrapper.displayName = `ComponentWrapper(${type})`;
  ...
  }, [debouncedSearchTerm, currentMatchIndex, setMatchRefs]);
      ...
      <ContentContainer>
        {fileError ? (
          ...
          showSource ? (
            ...
            <MarkdownContent>
              <div id='debug-info' style={{ display: 'none' }}>
                Search Term: {debouncedSearchTerm || 'none'}, Force Update:{' '}
                {forceUpdate}, Matches: {matches.length}
              </div>
              <ReactMarkdown
          ...
          <EmptyState>
            ...
            <p>
              Select a markdown or JSON file from the file browser to view its
              contents.
            </p>
          </EmptyState>
        )}
